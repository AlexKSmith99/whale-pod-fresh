-- Supabase Function Definitions
-- Generated: 2025-01-19
-- These are the actual function implementations in the database
...
RETURNS TABLE(pursuit_id UUID, pursuit_title TEXT, shared_meetings INTEGER) AS $$
BEGIN
  RETURN QUERY
  SELECT DISTINCT
    p.id AS pursuit_id,
...
-- NOTE: This function has a bug - references 'applications' table instead of 'pursuit_applications'
-- and 'connections' table with wrong column names (user_id vs user_id_1/user_id_2)
...
BEGIN
  ...
  -- Check 'connections'
  -- BUG: Uses wrong column names (user_id, connected_user_id instead of user_id_1, user_id_2)
  ...
  -- BUG: References 'applications' table instead of 'pursuit_applications'
  IF 'pod_creator_when_applying' = ANY(allowlist) THEN
    SELECT EXISTS (
      SELECT 1 FROM pursuit_applications a
