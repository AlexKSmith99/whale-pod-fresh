...
export default function EditPursuitScreen({ pursuit, onBack, onUpdated }: Props) {
  const { user } = useAuth();
  ...
  // Pre-populate fields with existing pursuit data
  useEffect(() => {
    if (pursuit) {
      setTitle(pursuit.title || '');
      setDescription(pursuit.description || '');
      setTeamSizeMin(String(pursuit.team_size_min || 2));
      setTeamSizeMax(String(pursuit.team_size_max || 8));
      setTeamSizeFlexible(pursuit.team_size_flexible || false);
      setLocation(pursuit.location || '');
      setProjectedDuration(pursuit.projected_duration || '');
      setSelectedTypes(pursuit.pursuit_types || []);
      setCategories(pursuit.pursuit_categories ? pursuit.pursuit_categories.join(', ') : '');
      setOwnershipStructure(pursuit.ownership_structure || '');

      // Convert decision_system back to display format
      const systemMap: any = {
        'standard_vote': 'Standard Vote',
        'admin_has_ultimate_say': 'Admin Has Ultimate Say',
        'delegated': 'Delegated',
        'weighted_voting': 'Weighted Voting',
      };
      setDecisionSystem(systemMap[pursuit.decision_system] || 'Standard Vote');

      setDecisionNote(pursuit.decision_system_note || '');
      setMeetingCadence(pursuit.meeting_cadence || '');
      setMeetingNote(pursuit.meeting_cadence_note || '');
      setAttendanceStyle(pursuit.attendance_style || 'Mandatory');
      setAttendanceNote(pursuit.attendance_note || '');
      setAccountabilityMechanics(pursuit.accountability_mechanics ? pursuit.accountability_mechanics.join(', ') : '');
      setRoles(pursuit.roles ? pursuit.roles.join(', ') : '');
      setExperienceLevel(pursuit.experience_level || '');
      setCurrentStage(pursuit.current_stage || '');
      setAgeRestriction(pursuit.age_restriction || '');
      setContinueAccepting(pursuit.continue_accepting_after_kickoff || false);
      setRequiresInterview(pursuit.requires_interview || false);
      setRequiresResume(pursuit.requires_resume || false);
      setApplicationQuestions(pursuit.application_questions ? pursuit.application_questions.join('\n') : '');
    }
  }, [pursuit]);
  ...
  const handleUpdate = async () => {
    ...
    setLoading(true);
    try {
      await pursuitService.updatePursuit(pursuit.id, {
        title,
        description,
      ...
      Alert.alert('✅ Success!', 'Your pursuit has been updated!', [
        { text: 'OK', onPress: onUpdated }
      ]);
    } catch (error: any) {
        ...
        <View style={styles.form}>
          ...
          >
            <Text style={styles.buttonText}>
              {loading ? '💾 Updating...' : '✨ Update Pursuit'}
            </Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
